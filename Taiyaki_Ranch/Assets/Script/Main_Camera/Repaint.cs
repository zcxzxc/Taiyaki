...
public class Repaint : MonoBehaviour
{
    ...
    private Vector2 Max = new Vector2(-1.2f, 2.25f); //y 좌표 랜덤 범위
    ...
    public void Paint() //리스트에 있는 붕어빵을 확인
    {
        ...
        for (int i=0;i<Data_base.Battle_Member.Length;i++)
            if (Data_base.Battle_Member[i] >= 0)
                Rendering(Data_base.Battle_Member[i],i);
    }
    ...
    private void Rendering(int Identity,int Slot) //리스트에 있을경우 생성
    {
        ...
        list[list.Count - 1].GetComponent<Sprite_change>().Member_Slot = Slot;
